...
public class TileEntityCharger extends TileEntity implements IInventory, IEnergyReceiver {
	...
	public void updateEntity() {
		...
		if (item == GameRegistry.findItem(Main.MODID, "itemIngotUmonia")) {
			if (!isActive)
				isActive = true;
			...
			if (progress >= getEnergizeTime()) {
				...
				progress = 0;
				if (item == GameRegistry.findItem(Main.MODID, "itemIngotUmonia"))
					setInventorySlotContents(0, new ItemStack(GameRegistry.findItem(Main.MODID, "itemIngotChargedUmonia")));
			}
		} else {
			if (isActive) {
				isActive = false;
