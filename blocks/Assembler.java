...
public class Assembler extends Block implements ITileEntityProvider {
	public Assembler() {
		super(Material.iron);
		setBlockName("blockAssembler");
		setBlockTextureName("JAMM:blockAssembler");
		setHardness(4.0f);
		setResistance(30.0f);
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase placer, ItemStack stack) {
        if (stack.hasDisplayName()) {
            ((TileEntityAssembler) world.getTileEntity(x,y,z)).setCustomName(stack.getDisplayName());
        }
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
        if (!world.isRemote) {
            player.openGui(Main.instance, GuiHandler.TILE_ENTITY_ASSEMBLER_GUI, world, x, y, z);
        }
        return true;
    }
}
